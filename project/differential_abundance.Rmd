---
title: "Differential abundance analysis - test"
output:
  html_document:
    toc: true
    toc_float: true
...
plot_bar(ps_genus, fill = "Genus") +
  facet_nested(~ Recent.antibiotics.use + Country, scales = "free_x") +
  theme_bw() +
...
dat_rel <- otu_rel %>%
  ...
  pivot_longer(-Sample, names_to = "taxon", values_to = "rel_abund") %>%
  left_join(meta_rel %>% as("data.frame") %>% rownames_to_column("Sample"), by = "Sample") %>%
...
dat_rel %>%
  ...
  ggplot(aes(x = Country, y = rel_abund, fill = Country)) +
...
dat_rel %>%
  filter(taxon == one_taxon) %>%
  ggplot(aes(x = Recent.antibiotics.use, y = rel_abund, fill = Country)) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(width = 0.15) +
    theme_bw()
```
...
wilcox_results <- dat_rel %>%
  group_by(taxon, Genus, Species) %>%
  summarise(
    p_value = wilcox.test(rel_abund ~ Country, exact = FALSE)$p.value,
    mean_dk = mean(rel_abund[Country == "Denmark"]),
    mean_usa = mean(rel_abund[Country == "United States of America"]),
    log2_fc = log2((mean_dk + 1e-6) / (mean_usa + 1e-6)),
    .groups = "drop"
  ) %>%
